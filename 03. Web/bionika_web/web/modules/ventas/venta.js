...
export async function buscarProductoPorClave() {
    ...
    let url = `http://localhost:8080/bionika_web/api/venta/getByClave/${encodeURIComponent(clave)}`;
...
export function agregarProductoAVenta() {
    ...
    // Buscar detalle que coincida con la talla seleccionada
    ...
    row.innerHTML = `
        ...
        <td class="border px-4 py-2">$${detalleVenta.precioUnitario.toFixed(2)}</td>
        <td class="border px-4 py-2">$${detalleVenta.total.toFixed(2)}</td>
...
async function generarNotaVentaPDF(venta) {
    ...
    doc.setFontSize(18);
    doc.text("Bionika - Nota de Venta", 105, 20, null, null, "center");
    ...
    const padding = 2;
    const datos = [
        { label: "No. de nota", valor: venta.idVenta },
        { label: "Cliente", valor: venta.cliente },
        { label: "Fecha", valor: new Date().toLocaleDateString() }
    ];

    let currentX = startX;
    const boxWidths = [50, 70, 50];
    ...
    datos.forEach((item, i) => {
        ...
        doc.roundedRect(currentX, startY, width, boxHeight, 3, 3);
        
        doc.text(`${item.label}:`, currentX + padding, startY + 6);
        doc.setFont("helvetica", "normal");
        doc.text(`${item.valor}`, currentX + padding + 25, startY + 6);
        doc.setFont("helvetica", "bold");
        currentX += width + 5; 
    });
    ...
    doc.autoTable({
        head: [["Producto", "Talla", "Unidad", "Cantidad", "Precio Unitario", "Total"]],
        body: rows,
        ...
        styles: {
            ...
            halign: 'center',
            valign: 'middle',
            lineColor: [140, 100, 200],
            lineWidth: 0.4
        },
        headStyles: {
            fillColor: [100, 70, 150],   // Morado fuerte
            textColor: 255,
            fontStyle: 'bold',
            halign: 'center'
        },
        ...
        alternateRowStyles: {
            fillColor: [245, 240, 255]  // Lila suave
        }
    });

    const finalY = doc.lastAutoTable.finalY || (startY + boxHeight + 20);

    // Total
    doc.setFontSize(13);
    doc.setTextColor(40, 40, 40);
    doc.setFont("helvetica", "bold");
    doc.text(`Total: $${venta.total.toFixed(2)}`, 150, finalY + 10);

    // Mensaje final
    doc.setTextColor(90, 50, 130);
    doc.setFontSize(12);
    doc.setFont("helvetica", "normal");
    doc.text("¡Gracias por tu compra!", 105, finalY + 25, null, null, "center");

    // Footer
    doc.setFontSize(9);
    doc.setTextColor(100, 100, 100);
    doc.text("Bionika | 5 de Mayo #725 Col. Obregon, León GTO, México. | Tel: (477) 713 1452", 105, 290, null, null, "center");

    // Guardar PDF
    doc.save(`nota_venta_${venta.idVenta}.pdf`);

    Swal.fire({
        title: 'Nota lista para imprimir',
        text: 'La nota de venta se ha generado correctamente',
        icon: 'success',
        showConfirmButton: false,
        timer: 2500,
